...
def test_repository_다국어_통합_suite는_한국어_gate와_다국어_suite를_그대로_결속한다() -> None:
    korean_path = Path("data/evaluation/ko-chat-quality-and-readiness-v1.jsonl")
    multilingual_path = Path("data/evaluation/multilingual-conversation-translation-v1.jsonl")
    combined_path = Path("data/evaluation/ko-multilingual-chat-quality-v1.jsonl")
    expected = korean_path.read_bytes() + multilingual_path.read_bytes()
    assert combined_path.read_bytes() == expected
    assert sha256_file(combined_path) == (
        "bd76a4333ef5a423e3f22733ed85100178b0c38ac44f8881d3a7b6c4b6c3fb23"
    )
    ...
    prompts = [turn.user for row in rows for turn in row.turns]
    assert len(prompts) == 65
    assert len(set(prompts)) == 65
    assert sum(len(row.turns) for row in rows) * 6 == 390
