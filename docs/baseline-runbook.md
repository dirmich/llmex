...
## 1.4.0 nonce 환경 계약과 최종 재검증

external command는 실행 중 `LLMEX_STAGE_NONCE`, `LLMEX_RUN_ID`, `LLMEX_STAGE_NAME`,
`LLMEX_BUDGET_JSON`, `LLMEX_GIT_COMMIT`, `LLMEX_CONFIG_FINGERPRINT`를 읽어 실제 측정이 끝난 뒤
`LLMEX_TELEMETRY_PATH`에 final 진술을 서명한다. 미리 서명한 template 복사는 허용되지 않는다.
`issued_at`은 stage 시작 이후여야 하고 `expires_at`은 검증 시점에 유효해야 한다. 후속 local stage가
있더라도 최종 성공 직전에 마지막 권위 telemetry의 digest와 전체 계약을 다시 검증하므로 파일을
변경하지 않아야 한다.
