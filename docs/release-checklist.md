...
**1.22.21 source 결속 teacher 응답 품질 gate·conversation act 계약·번역 분포 조기 감사·로컬 HF/GGUF export·통합 자동 품질 suite·자유대화 smoke·독립 수동 검토**다. Qwen natural-v4 v3는 310건에서 세 번역 방향 accepted 0을 확인해 중단·기각했으며 resume·export·학습 입력 사용을 금지한다. 다음 fresh 계약은 자연스러운 동의어·활용형을 포함하되 숫자·entity·핵심 의미 보존을 계속 실패-폐쇄해야 한다. 이번 실행에서는 Hugging Face에 공개·비공개 모두 업로드하지 않으며 validation PPL 감소나 aggregate 품질만으로 checkpoint를 승인하지 않는다.
