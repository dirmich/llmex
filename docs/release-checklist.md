...
**1.22.18 source 결속 teacher 응답 품질 gate·600-step 혼합 SFT·focused-v12 150-step repair·로컬 HF/GGUF export·통합 자동 품질 suite·자유대화 smoke·독립 수동 검토·외부 공개 승인 전 공개 금지**다. focused-v12와 결함 있는 expanded tranche·Gemma 한국어 v2는 기각됐다. Qwen 다국어 v2는 재시도 실패 1건을 해소하기 전 감사·export를 차단하고, Gemma 한국어 v3는 강화 gate로 재개한다. 이번 실행에서는 Hugging Face 업로드를 하지 않으며 validation PPL 감소나 aggregate 품질만으로 checkpoint를 승인하지 않는다.
