...
처음 학습할 때는 [20장 offline 대화 E2E](20-offline-chat-e2e.md)의 생성기를 실행해 실제 파일과 SHA가 결속된 교재 설정을 만든다.
...
uv run llmex sft eval --config runs/book-chat-smoke/configs/sft.yaml --checkpoint runs/book-chat-smoke/sft/checkpoints/latest.pt
uv run llmex sft generate --config runs/book-chat-smoke/configs/sft.yaml --checkpoint runs/book-chat-smoke/sft/checkpoints/latest.pt --prompt "안녕하세요"
...
교재 smoke checkpoint를 만들었다면 checkpoint bytes에서 SHA 결속 설정을 먼저 생성한다.
...
```bash
uv run llmex sft quality-review-template --config runs/book-chat-smoke/configs/quality.yaml
uv run llmex sft quality-review-validate \
  --config runs/book-chat-smoke/configs/quality.yaml \
  --repository . \
