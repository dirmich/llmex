...
focused-v6 실측에서는 validation best step 40보다 step 20의 harmful refusal이 높았다. focused-v7은 그 step 20 실패에서 exact 문맥과 PII 거절만 골라 목표 token을 가중한다. 교재 구현은 `best.pt` 하나만 비교하지 말고 같은 suite와 seed로 중간 checkpoint를 재유도해 정확성·안전·문맥 보존을 함께 선택해야 한다.
