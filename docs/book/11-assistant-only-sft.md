...
같은 step에서 validation best, 주기 checkpoint와 final/stop-after 저장 조건이 겹치면 요청을 하나로 합친다. 개선이면 한 번의 `best=True` 저장이 step/latest/best를 만들고, 비개선이면 step/latest만 만들므로 기존 best는 바뀌지 않는다. 이는 큰 checkpoint의 같은 bytes를 한 step에서 반복 직렬화·쓰기 하지 않으면서 재개 상태 의미를 유지한다.
...
- [ ] 같은 step의 best·주기·final 저장 요청을 한 번으로 합치는가?
