...
개념을 순서대로 공부하려면 00~15장을 읽고, 특정 파일의 책임을 찾으려면 16장을 색인으로 사용한다. 완성 코드를 보지 않고 전체 제품을 직접 만들어 보려면 17장의 0~12단계를 수행하고, Python 파일 하나씩 구현하려면 19장의 여섯 모듈 챕터를 사용한 뒤 원본 모듈과 비교한다.
