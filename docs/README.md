...
현재 상태: 1.17.0. focused-v7에서 고정된 최신 날짜 설명 문장 오류를 일반화해 날짜·배포 코드·담당자·승인 상태·회의 장소의 “갱신 뒤 값만 출력” focused-v8을 생성했다. train 8,400/heldout 840행, suite·split 모든 user turn과 source overlap 0이며 manifest fingerprint는 `f4dc0633…d647`다. 다음 단계는 v7 안전 checkpoint 기반 짧은 SFT와 고정 162응답 재평가다. 교재에는 57개 모듈 카드와 챕터별 환경 준비표, 실제 실행 가능한 offline 공개+teacher mix→CPU SFT→추론→자동 품질 실습을 포함한다. 내부 teacher checkpoint를 base로 한 모든 후속 SFT는 원 release block을 계승한다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 SHA·실행 결과와 외부 승인 경계를 기록한다.
