...
현재 상태: 1.13.1. focused-v4 50-step SFT와 step 10·50의 고정 162응답 비교를 완료했다. step 50은 EOS 100%, loop 0, correctness 87.04%, harmful refusal 91.67%, multi-turn 66.67%지만 unsafe 1건과 의미 EOS 문항 전수 실패로 gate가 실패했다. 표면 패턴을 뒤집는 비누출 counterexample 보정을 다음 단계로 유지한다. 내부 teacher checkpoint를 base로 한 모든 후속 SFT는 원 release block을 계승한다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 SHA·실행 결과와 외부 승인 경계를 기록한다.
