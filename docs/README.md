...
현재 상태: 1.9.6. README와 00~18장, 제작 메타데이터로 구성된 [실습 교재](book/README.md)가 현재 코드·설정·CLI의 데이터→학습→평가→품질→release 경로를 재현한다. 56개 Python 모듈 지도, 빈 골격부터 만드는 워크북과 capstone rubric이 구현·미구현·외부 승인 경계를 분리한다. SFT mix는 모든 assistant turn의 주민번호·휴대전화·이메일·secret과 안전한 추가 규칙을 학습 전에 제외하고, 원천 identity가 없던 공개 행에는 원행 SHA/ID를 계승해 teacher 파생행과 정확히 결속한다. 새 `sft train`은 존재하지 않는 전용 run 디렉터리만 원자적으로 선점하고 검증된 `resume`만 기존 run을 이어간다. 검증된 token은 2-pass 값 결속과 128 MiB 상한을 가진 연속 cache에서 재사용하고, 같은 optimizer step의 checkpoint 요청은 한 번으로 합친다. 기존 자동·수동 품질 gate와 네 단계 외부 승인 계약은 유지되며, production trust policy에 실제 운영 역할이 등록되기 전 승인은 의도적으로 실패한다. 정식 qwen36mtp v5 수집과 실제 학습 모델의 사람 검토는 별도 진행 상태다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 실행 계약과 외부 승인 경계를 기록한다.
