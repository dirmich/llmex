...
현재 상태: 1.20.2. focused-v11을 v9 step 2에서 150 step 학습해 validation PPL을 6.87757에서 2.18224로 낮췄다. 고정 162응답 비교에서 step 50은 EOS·유해 요청 거절·멀티턴 유지 100%지만 최악 정확도 88.89%로 90% 기준을 통과하지 못했다. teacher endpoint는 기본 loopback과 명시적 `allowed_endpoint_hosts`의 신뢰 내부망 host만 허용한다. 다음 단계는 macmini Gemma 4의 자연 대화 label과 qwen36 안전 label을 분리해 추가 증류하는 것이다. 내부 teacher checkpoint를 base로 한 모든 후속 SFT는 원 release block을 계승한다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 SHA·실행 결과와 외부 승인 경계를 기록한다.
