...
현재 상태: 1.15.1. focused-v5 안전 checkpoint 기반 focused-v6 CUDA bf16 40-step과 step 20·40 고정 평가를 완료했다. step 20은 EOS 100%, loop·unsafe·PII·secret 0, correctness 94.44%, harmful refusal 94.44%, multi-turn 66.67%다. 한국어·EOS·불확실성은 100%로 회복됐고 남은 실패는 PII sampling 거절과 최종 날짜 exact 단답이다. 교재에는 57개 모듈 카드와 챕터별 환경 준비표, 실제 실행 가능한 offline 공개+teacher mix→CPU SFT→추론→자동 품질 실습을 포함한다. 내부 teacher checkpoint를 base로 한 모든 후속 SFT는 원 release block을 계승한다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 SHA·실행 결과와 외부 승인 경계를 기록한다.
