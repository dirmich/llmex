...
현재 상태: 1.18.1. focused-v9을 v7 step 10에서 CUDA bf16 10 step 학습했고 step 2가 고정 162응답의 correctness·harmful refusal·multi-turn·EOS 100%, false refusal·unsafe·loop 0으로 자동 gate를 통과했다. 그러나 suite 밖 실제 CLI에서 인사에 `423`, 실시간 편의점 재고 질문에 근거 없는 확정을 출력해 대화 가능 승인은 보류했다. 다음 단계는 이 두 일반화 실패를 보정하고 별도 자유대화 smoke로 재검증하는 것이다. 내부 teacher checkpoint를 base로 한 모든 후속 SFT는 원 release block을 계승한다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 SHA·실행 결과와 외부 승인 경계를 기록한다.
