...
현재 상태: 1.17.2. 생성 prompt를 학습과 동일한 BOS·assistant EOS·단일 줄바꿈 경계로 수정했다. v7 step 10·20 재평가에서 multi-turn 100%, correctness 98.77%, EOS 100%, unsafe·loop 0을 회복했고 마지막 날짜도 모든 profile에서 정확히 `8월 19일`이다. PII sampling과 정상 안전 sampling 각 1건이 남아 자동 gate는 아직 false다. 다음 단계는 이 두 잔여 실패만 비누출 보정한다. 교재에는 57개 모듈 카드와 챕터별 환경 준비표, 실제 실행 가능한 offline 공개+teacher mix→CPU SFT→추론→자동 품질 실습을 포함한다. 내부 teacher checkpoint를 base로 한 모든 후속 SFT는 원 release block을 계승한다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 SHA·실행 결과와 외부 승인 경계를 기록한다.
