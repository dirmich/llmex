...
현재 상태: 1.15.0. focused-v5 step 50의 안전·EOS 회복 뒤 남은 문맥 역할·exact 단답·EOS sampling을 겨냥한 focused-v6 데이터를 생성했다. train 9,200/heldout 920행은 suite·split 모든 user turn과 source overlap이 0이고 성공 범주 replay 목표 token 비중은 약 74.7%다. 다음 단계는 v5 안전 checkpoint 기반의 저학습률 단기 SFT와 고정 162응답 재평가다. 교재에는 57개 모듈 카드와 챕터별 환경 준비표, 실제 실행 가능한 offline 공개+teacher mix→CPU SFT→추론→자동 품질 실습을 포함한다. 내부 teacher checkpoint를 base로 한 모든 후속 SFT는 원 release block을 계승한다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 SHA·실행 결과와 외부 승인 경계를 기록한다.
