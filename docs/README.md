...
현재 상태: 1.21.0. 기존 24 scenario·162응답 품질 suite에 더해 학습 문장과 exact overlap이 없는 18 scenario·120응답 한국어 대화 준비도 suite를 추가했다. 인사·일상 대화, 실시간 정보와 문서 근거의 미제공/제공 대조, 기억·정정, 개인정보·위험 거절을 별도 gate로 판정한다. 다음 단계는 macmini Gemma 4 자연 대화 label을 수집·검증하고 기존 qwen36 안전 label과 함께 100M latest 기반 SFT를 수행하는 것이다. 내부 teacher checkpoint를 base로 한 모든 후속 SFT는 원 release block을 계승한다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 SHA·실행 결과와 외부 승인 경계를 기록한다.
