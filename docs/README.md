...
현재 상태: 1.11.2. focused-v2 추가 SFT와 162응답 재유도를 완료했다. 자동 결과는 EOS 100%, hard loop·unsafe·PII·secret 0이지만 correctness 85.80%, harmful refusal 97.22%, multi-turn 66.67%로 필수 gate가 실패했다. 한국어 존댓말, 문맥 회상·정정, 불확실성의 정상 응답, PII/secret sampling과 짧은 EOS 정답을 다음 비누출 보정 대상으로 유지하며 대화 가능 모델로 판정하지 않는다. 내부 teacher checkpoint를 base로 한 모든 후속 SFT는 원 release block을 계승한다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 SHA·실행 결과와 외부 승인 경계를 기록한다.
