...
현재 상태: 1.9.1. README와 00~15장으로 구성된 [실습 교재](book/README.md)가 현재 코드·설정·CLI의 데이터→학습→평가→품질→release 경로를 재현한다. SFT mix는 모든 assistant turn의 주민번호·휴대전화·이메일·secret과 안전한 추가 규칙을 학습 전에 제외한다. 기존 자동·수동 품질 gate와 네 단계 외부 승인 계약은 유지되며, production trust policy에 실제 운영 역할이 등록되기 전 승인은 의도적으로 실패한다. 정식 qwen36mtp v5 수집과 실제 학습 모델의 사람 검토는 별도 진행 상태다. [실행 가이드](run-guide.md), [한국어 대화 SFT 가이드](chat-sft.md), [teacher 증류 데이터 실행 가이드](teacher-distillation.md), [M7 릴리스 체크리스트](release-checklist.md)에 실행 계약과 외부 승인 경계를 기록한다.
