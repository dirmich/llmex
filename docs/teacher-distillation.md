...
두 source는 teacher별 6,000행이 모두 고유하고 train/heldout prompt overlap이 0이다. 아래 v3 명령은 과거 상태 재현 전용이다. `collect`, `audit-sample --approve`, `export`, `validate`를 실행하지 않는다.

```bash
uv run llmex distill status \
  --config configs/distill/qwen36mtp-multilingual-natural-2000-v3-rejected.yaml
```
