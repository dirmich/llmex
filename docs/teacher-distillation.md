...
LLMEX 1.6.1의 teacher 증류 경로는 로컬 OpenAI 호환 서버에서 한국어 응답을 수집해 assistant-only SFT 입력을 만든다. 현재 정식 실행은 `configs/distill/qwen36mtp-10k.yaml`, teacher는 `http://localhost:8081/v1`의 `qwen36mtp`, run은 `runs/distill/qwen36mtp-10k-v5`다. v5 30건 pilot은 전체 단계를 통과했고 정식 inventory 10,000건과 preflight는 준비됐지만 실제 10k 수집은 아직 시작하지 않았다.
...
- 현재 status: preflight 통과, total 10,000, pending 10,000, completed 0, progress 0
