...
LLMEX 1.22.21은 Wikipedia 사전학습과 분리된 assistant-only 대화 학습, 여러 teacher를 직접 결속하는 비누출 mix, 결정적 능력 보정 curriculum, fresh SFT 실행 경계, 상한이 있는 token cache와 자동·수동 품질 gate를 제공한다. focused-v12, Qwen 다국어 v2, 세 번역 방향 accepted 0인 Qwen natural-v4 v3는 사용하지 않는다. 다음 학습은 대화 행위와 번역 동의어·활용형 계약 및 독립 표본 감사를 모두 통과한 fresh Qwen·Gemma export만 사용하며, 기각 run이나 미승인 spool은 혼합하지 않는다.
