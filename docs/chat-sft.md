...
LLMEX 1.22.8은 Wikipedia 사전학습과 분리된 assistant-only 대화 학습, 여러 teacher를 직접 결속하는 비누출 mix, 결정적 능력 보정 curriculum, fresh SFT 실행 경계, 상한이 있는 token cache와 자동·수동 품질 gate를 제공한다. 600-step 혼합 SFT의 390응답 실패 뒤 suite 문장을 복제하지 않은 focused-v12 train 4,000·heldout 400행을 준비했다. 실제 사람 품질·법무·외부 공개 승인 전에 A/B와 집중 재학습을 통과해야 한다.
