...
LLMEX 1.22.19는 Wikipedia 사전학습과 분리된 assistant-only 대화 학습, 여러 teacher를 직접 결속하는 비누출 mix, 결정적 능력 보정 curriculum, fresh SFT 실행 경계, 상한이 있는 token cache와 자동·수동 품질 gate를 제공한다. focused-v12는 150 step까지 학습했지만 자동 gate와 suite 밖 실제 다국어 대화를 실패했다. Qwen 다국어 v2도 독립 50건 표본 감사를 실패해 사용하지 않는다. 다음 학습은 conversation act 계약과 독립 표본 감사를 모두 통과한 새 Qwen tranche와 Gemma 한국어 v3 export만 사용한다.
...
focused-v12는 단일 replay 파일의 무작위 비중 대신 이름 있는 원천별 quota를 사용한다. 신규 합성 2,000행, Qwen 다국어 799행, Gemma 다국어 733행과 안전·불확실성 보존 468행을 결합한다. generic Carrot·public-other 장문 replay는 제외한다. train/heldout SHA는 `e3483f14…ff33`·`c444720c…0fc8`, manifest SHA는 `f2794728…8deb`이며 통합 suite·split·source overlap은 0이다.

25-step A/B에서 4e-6은 unsafe 0·EOS 100%·유해 거절 93.75%를 기록했지만 정확도 28.46%, 멀티턴 6.67%, 정상 오거절 13.45%, loop 1건으로 gate를 실패했다. 2e-6은 unsafe 2건 때문에 우선 탈락했다. 정식 학습은 A/B를 resume하지 않고 step 600 원본 가중치에서 4e-6으로 다시 시작한다.

정식 150-step은 안전과 문맥을 개선했지만 step 150에서 정확도 37.69%, 멀티턴 46.67%, 정상 오거절 11.99%, loop 4건이었다. suite 밖 한국어·영어·일본어도 자연스럽지 않았다. 따라서 heldout PPL 1.40665와 유해 거절 100%만으로 승인하지 않으며, direct 다국어 teacher train 1,532행의 규모 부족을 다음 데이터 단계에서 해소한다.
