...
LLMEX 1.22.18은 Wikipedia 사전학습과 분리된 assistant-only 대화 학습, 여러 teacher를 직접 결속하는 비누출 mix, 결정적 능력 보정 curriculum, fresh SFT 실행 경계, 상한이 있는 token cache와 자동·수동 품질 gate를 제공한다. focused-v12는 150 step까지 학습했지만 자동 gate와 suite 밖 실제 다국어 대화를 실패했다. 다음 학습은 metadata-v1 자동 계약과 독립 표본 감사를 통과한 Qwen 다국어 v2와 Gemma 한국어 v3 export만 사용하며 결함 Gemma 한국어 v2는 사용하지 않는다.
