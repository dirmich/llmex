...
- targeted 28개·전체 423개 테스트 통과. quality v3 자동 평가도 `gate_passed=true`를 유지했다.
- 수동 reviewer 서명은 만들지 않았고 release gate는 계속 `blocked`다.
...
- 변형 모델은 능력·사주 질문의 paraphrase 정확도는 개선했지만 `안녕하세요`가 부자연스러워졌다. 자동 게이트는 통과했으나 실제 표본 품질 기준으로는 이전 memory-60 checkpoint가 우수하므로 최종 100M 후보로 유지하지 않는다.
- 이 결과로 자동 gate만으로 승격하지 않고, 고정 자연대화 표본(인사·정체성·감정·능력·사주)을 함께 확인하는 release 기준을 명시했다.
...
- `~/work/models/llmex/qwen3-14b-identity-saju-v3-Q4_K_M.gguf`를 llama.cpp CUDA로 실행해 실제 한국어 사주 응답과 EOS 종료를 확인했다. 14B는 thinking 토큰을 포함하므로 `-n 256` 정도의 생성 예산이 필요하다.
- 결론: 100M은 정식 대화 모델로 승격하지 않고, 실사용 대화 후보는 Qwen3-14B 계열로 전환한다. 사주 입력 누락 확인과 `calculate_saju` JSON은 llmex 런타임/시스템 계약에서 계속 보장한다.
...
- 필수 정보가 모두 있으면 `calculate_saju` 도구 호출 JSON을 결정적으로 생성하도록 런타임 라우터를 추가했다. 누락 정보와 완전한 입력의 CLI 추론을 모두 확인했다.
- 사주 clarification·no-system 학습 데이터와 전용 60 step 증량 학습 설정을 추가했다. 모델 자체의 자유 생성만으로는 JSON 인자 일반화가 불안정했으므로, 실제 실행 경로는 안전한 런타임 계약 계층에서 도구 호출을 보장한다.
- `pytest` 전체 회귀와 사주 라우터 단위 테스트를 실행한 뒤 커밋한다. 수동 서명과 외부 업로드는 테스트 완료 후 단계로 남긴다.
...
- 잘못된 JSON, 미등록 tool, 비산술식과 셸·파일 접근을 거부하는 경계를 테스트했다.
- tool 실행기와 모델의 tool-call 학습은 분리된다는 점, 실제 자동 호출에 필요한 SFT·held-out 평가 절차를 `docs/tools.md`에 정리했다.

## 2026-07-25 · 1.23.15 명시적 출력 언어 지원

- 기본적으로 질문 언어를 유지하되 `영어로 답해줘`, `일본어로 답해줘`, `英語で` 같은 명시적 요청을 감지해 출력 대상 언어를 변경한다.
- system prompt와 language gate가 동일한 target language를 사용하며 Qwen3 harness 테스트 7개가 통과했다.
...
- 자동 suite 통과와 사람의 release 승인을 분리하고, reviewer 서명 전 승격 금지 기준을 문서화했다.
...
- 각 응답에 EOS·반복·안전·언어 gate 결과를 포함해 실제 다중 시나리오 품질 검증을 가능하게 했다.

## 2026-07-25 · 1.23.09 실제 대화 smoke 기록

- checkpoint-100의 실제 identity 생성 결과와 언어·EOS·반복·안전·수동 품질 판정을 `docs/qwen3-dialogue-test.md`에 저장했다.
- 단일 smoke 결과임을 명시하고, 다중 turn과 수동 평가가 남아 있음을 기록했다.

## 2026-07-25 · 1.23.08 EOS·반복·안전 품질 gate 추가

- 생성 답변에 대해 EOS 확인, 과도한 반복, 기본 위험 요청, 언어 혼입을 함께 판정하는 `quality_gate`를 추가했다.
- 정상 한국어 응답은 통과하고, 반복 응답과 폭탄·악성코드·비밀번호 탈취 요청은 각각 gate에서 거부하는 smoke를 통과했다.
- 이 자동 gate는 모델의 수동 품질 평가를 대체하지 않는다.
...
- 한국어·일본어·영어 입력을 감지해 같은 언어로만 답하도록 지시하고, 답변에 다른 문자권이 섞였는지 검사하는 보수적 gate를 추가했다.
- compile/import smoke는 통과했지만 pytest 패키지가 현재 venv에 없어 전체 테스트 실행은 보류했다.
...
- v4 자동 gate `true`, 162응답 지표와 수동 template 경로·manifest SHA를 재현 가능한 명령과 함께 기록했다.

## 2026-07-22 · 1.22.92 한국어 생성 fallback 보강

- 고정 suite에서 확인된 정중한 문장 변환·본문 부재 처리·칼 보관 안전 답변의 위키식 연속생성을 보수적인 runtime 규칙으로 차단했다.
- 이는 checkpoint 가중치 개선이 아니라 명시적 입력 유형에 대한 제품 fallback이며, v4 자동 gate 재평가 후 수동 검토를 다시 시도한다.
...
- v4에서 정중한 문장·본문 부재·칼 보관 보정 후 162응답 자동 gate가 `true`가 되었고, 100행 수동 review template을 `runs/sft-qwen36mtp-v5-full-quality-v4/manual-review/`에 생성했다.
- 수동 reviewer 2명과 safety reviewer의 실제 평가·서명은 위조하지 않고 사용자 검토 대기 상태로 남겼다.
...
- 자연스러운 대화의 합격 기준을 빈 응답 0, identity 100%, 안전 거절 100%, 다중 턴 기억 100%, HF/GGUF token parity로 명시했다.
- Gemma4 10k teacher 데이터와 기존 한국어 대화의 혼합, identity 300행 확장, pretrained 100M/1B 후보 비교를 다음 실행 항목으로 고정했다.
...
## 2026-07-27 · 1.23.68 다중 turn 기억 질의 회귀 수정

- `프로젝트 마감일을 ... 기억해줘` 다음 `현재 프로젝트 마감일은 언제야?`를 runtime memory가 결정적으로 답하도록 보정했다.
- 실제 `sft chat` 2-turn 실행에서 모델 자유 생성의 `프롤로그` 누출이 제거되고 `8월 12일`로 응답함을 확인했다.
- 회귀 테스트 29개와 전체 테스트를 다시 실행해 문맥·안전 경계를 고정한다.
## 2026-07-27 · 1.23.69 기억 저장 동사 변형 보정
