...
- 영어·일본어→한국어 번역 계약에 수령인 이름의 `에게/께` 보존 조건을 추가하고 회귀 테스트를 통과시켰다.
- Hugging Face 공개·비공개 업로드는 수행하지 않았다.
...
- task 균등 결정적 50건을 승인 전에 전수 검토해 질문·제안·비반복 지시 불이행과 부자연스러운 조언을 확인했다.
- 표본 감사 artifact를 `approved=false`로 결속해 이 tranche의 export와 학습 입력 사용을 차단했다.
...
- 직접 부정 안전 안내는 허용하는 대조 회귀를 추가했다.

## 1.22.16 - 2026-07-18

- 지도 서비스 혼잡도 정보에 의존하지 말라는 안전한 부정문은 보존하면서 긍정 참고·제공 우회만 거절하도록 극성을 구분했다.
- Qwen 다국어 v2와 Gemma 한국어 v3 운영 경로, 결함 Gemma 한국어 v2 격리 상태를 모든 실행 문서에 맞췄다.

## 1.22.15 - 2026-07-18

- Gemma 한국어 v2 조기 표본에서 확인한 지도 서비스 혼잡도 참고·제공 우회 주장을 uncertainty 품질 gate가 거절하도록 강화했다.
- 실제 teacher 응답 두 건을 회귀로 고정하고 기존 spool 재검증에도 같은 판정을 적용했다.
...
- 첫 품질 판정을 앞당기기 위해 한국어 3,000·Qwen/Gemma 다국어 각 2,000의 재현 가능한 1차 tranche 설정을 추가했다.
- 1차 학습이 부족하면 이미 준비·부분 수집된 10,000/6,000 full spool을 그대로 재개하도록 경로를 분리했다.
...
- step 50·150을 각각 전체 390응답으로 평가·재유도해 안전·문맥 개선과 정확도·오거절·반복 실패를 고정했다.
- suite 밖 한국어·영어·일본어 자유대화 smoke를 실행해 실제 비문을 확인하고 최종 checkpoint를 기각했다.
...
- EOS·유해 거절 100%에도 정확도 45%, 최악 35%, 멀티턴 0%로 실패한 기준선을 기록했다.
- 인사 오거절과 일상 비문을 새 Gemma4 혼합 SFT의 필수 개선 대상으로 고정했다.
...
- 안전 복원 step 20의 고정 162응답이 최악 정확도·유해 거절·unsafe gate를 실패해 checkpoint를 기각했다.
- 다음 정식 학습 시작점을 100M latest와 Qwen/public·Gemma·안전 replay 결합으로 고정했다.

## 1.20.4 - 2026-07-18

- 말단 CR/LF가 있는 대화 메시지를 학습과 추론에서 동일하게 정규화한다.
- 실제 assistant 이력의 생성 prompt 토큰과 학습 prefix 토큰 완전 일치를 회귀로 고정했다.
- Gemma 4 추가 증류와 100M latest 기반 SFT 전에도 템플릿 불일치로 인한 다중 턴 왜곡을 차단한다.
...
- 적용된 decoding 설정과 seed를 생성 결과에 기록해 실제 응답 재현성을 높였다.
- 명시적 generation 설정과 고정 generator를 runtime 회귀로 검증했다.
...
- step 5·20의 고정 162응답을 생성·재유도했다.
- 반복된 다중 턴 오류의 원인을 학습 assistant EOS와 생성 prompt EOS의 템플릿 불일치로 확정하고 기존 평가를 승인 근거에서 제외했다.
...
- 환경 설정부터 모듈별 구현·검증·기록으로 이어지는 19장 학습 경로를 추가했다.
- 소스 Python 모듈과 교재 카드의 정확한 일대일 대응, 모듈 챕터 링크를 자동 검증하는 문서 회귀 테스트를 추가했다.
